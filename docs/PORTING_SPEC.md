...
| 窗口操作 | 原生窗口菜单优先 + 稳定目标锁定 + 约束感知的 `AXUIElement` 降级 | 辅助功能 | P2A | 中：非标准窗口、分离进程与最小尺寸约束 |
...
| 软件更新 | Sparkle + Ed25519 appcast + GitHub Releases | 网络 | P3 | 中：分发签名与公证 |
...
- 窗口居中、填充桌面、左右/上下/四角贴靠、恢复原大小、全屏切换、最小化、关闭、关闭所有类似窗口、退出应用，以及左亮度/右音量边缘滚动。
- 窗口动作在手势开始时锁定输入目标，并将输入进程与菜单/逻辑应用分开解析；正常应用优先调用原生“窗口”菜单，菜单缺失或行为不完整时才使用 Accessibility。
- Accessibility 几何降级采用“位置 → 尺寸 → 位置”写入，并根据应用实际最小尺寸将窗口锚定在目标边缘，避免贴靠后回弹或漂移。
- Steam 等多进程应用由逻辑主进程负责退出、界面进程负责窗口操作；窗口关闭后将焦点交接给后方应用，最后一个可见窗口关闭时切换至 Finder。带关闭动画的应用只做短时、有界的状态检查。
...
- 登录启动已完成。Sparkle 负责每日/手动更新检查、Ed25519 完整性验证、下载与重启安装；tag 驱动发布会生成 GitHub Pages appcast。
- 开发机通过 Apple Development 签名保持权限身份稳定；公开 DMG 仍未完成 Developer ID Application 签名与 Apple 公证。Sparkle 可安全验证更新包，但公开 ad-hoc 签名仍可能导致 macOS 隐私权限在覆盖更新后需要重新授予。
...
- Accessibility 对 Electron、自绘窗口和系统受保护界面的支持不一致；当前使用原生菜单优先、分离进程解析与约束感知几何降级，但受应用最小尺寸限制时无法保证数学意义上的精确 50% 分屏。
