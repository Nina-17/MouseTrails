...
| SwiftUI 设置与教程 | 配置编辑、手势映射、权限诊断、定向跳转、授权助手与安全练习模式 | 无 | P2B | 中：与配置 schema 和手势事件链同步 |
...
`bundleIdentifiers` 为空表示全局规则；非空时优先匹配特定应用。当前 schema 为 v5：无 `schemaVersion` 的旧平铺配置按 v1 读取，v2–v4 配置会逐项补全安全默认值，下次保存时写回 v5；`customGestures` 缺失时迁移为空数组。每个自定义手势保存 3 组、64 点归一化样本。高于当前版本的配置不做猜测解析。
...
- 八方向、S/W、顺逆时针方框与三样本可训练手势。
- 在 P2-0 动作序列上扩展按键按下/抬起、剪贴板和窗口动作。
- 窗口最大化/居中/关闭/置顶、Option 拖动、边缘滚轮和音量。
- 剪贴板菜单、连续复制与快速粘贴。

### 5.2 P2B：视觉与设置

- SwiftUI 设置页、手势编辑器、配置导入和权限诊断。
- ScreenCaptureKit 框选截图、窗口截图与贴图。
...
- Developer ID Application 签名与 Apple 公证仍未完成；在此之前不做静默自替换。
