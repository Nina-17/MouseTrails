...
`bundleIdentifiers` 为空表示全局规则；非空时优先匹配特定应用。当前 schema 为 v3：无 `schemaVersion` 的旧平铺配置按 v1 读取，v2 配置会补全安全的动作序列默认值，下次保存时写回 v3；高于当前版本的配置不做猜测解析。后续兼容层再负责把 Windows 进程名和 `SendKeys` 动作转换到此模型。
